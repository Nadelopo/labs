...
public:
	...
	void setСoordinates(int _x, int _y) {
		x += _x; y += _y;
	}
	void getCoordinates() {
...
public:
	...
	void updateСoordinates(int _x, int _y) {
		x1 += _x; y1 += _y;
		x2 += _x; y2 += _y;
		x3 += _x; y3 += _y;
		x4 += _x; y4 += _y;
...
int main() {
	...
	do {
		...
		case 1:
			...
			cout << "укажите координату y: "; cin >> y; c1.setСoordinates(x, y);
			c1.getRadius();
			c1.getCoordinates();
			break;
		case 2:
		...
		if (method == 1) {
			...
			if (figure == 1) {
				c1.setСoordinates(x, y);
				c1.getCoordinates();
			}
			if (figure == 2) {
				s1.updateСoordinates(x, y);
				s1.getCoordinates();
			}
			if (figure == 3) {
				r1.updateСoordinates(x, y);
				r1.getCoordinates();
			}
		}
		if (method == 2) {
